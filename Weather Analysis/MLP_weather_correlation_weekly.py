...
def predict_function(trial, X_train, X_test, y_train, y_test, test_dates, file):
    ...
    if file == "concelho_metropolitana_lisboa.csv":
        # Hidden Layer 0
        model.add(Dense(112, activation=activation_layer, input_dim=X_train.shape[1]))
        # Hidden Layer 1
        model.add(Dense(72, activation=activation_layer))
                # Hidden Layer 2
...
if __name__ == "__main__":
    ...
    pivot = pd.pivot_table(df2, 
                       index=pd.Grouper(key='data_saida', freq="W"),
    ...
    merged_df = pd.merge(group_by_week, pivot, on='data_saida', how='left')
    print("Merged")
    ...
    if file =="concelho_metropolitana_lisboa.csv":
        storage_url = "sqlite:///MLP_lisboa_CORRELATION_WEEKLY.sqlite3"
    else:
        storage_url = "sqlite:///MLP_porto_CORRELATION_WEEKLY.sqlite3"
